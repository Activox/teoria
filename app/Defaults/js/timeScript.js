    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (data.product.code == 400) {
                ...
                let title = "Productos mas tiempo de produccion";
                let myData = [];
                $.each($.parseJSON(data.product.chart), function (key, value) {
                    myData.push([value.product_name, (value.product_time / data.product.total_time)]);
                });
            ...
            if (data.style.code == 400) {
                ...
                let title = "Estilos mas tiempo de produccion";
                let myData = [];
                $.each($.parseJSON(data.style.chart), function (key, value) {
                    myData.push([value.style_name, (value.style_time / data.style.total_time)]);
                });
            ...
            if (data.module.code == 400) {
                ...
                let title = "Modulos mas tiempo de produccion";
                let myData = [];
                $.each($.parseJSON(data.module.chart), function (key, value) {
                    myData.push([value.module, (value.modulo_time / data.module.total_time)]);
                });
