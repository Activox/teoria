...
class FileHelper {
	...
	private static $_maxSize_		=	31457280;
	
	private static $_extensions_	=	array('png', 'jpg', 'jpeg');
	
	private static $_mimeTypes_	=	array('image/png', 'image/jpeg', 'image/pjpeg', 'image/jpeg', 'image/pjpeg');
    ...
    public static function uploadFile($index = 'logo', $fileName = '', $options = array()) {
        ...
        if (!is_string($index) || !is_string($fileName) || !is_array($options)) {
			throw new Exception( ucfirst(Lang::get("error.msg_10")) );
            //return false;
        }

        $maxSize 		= (isset($options['maxSize']) && is_int($options['maxSize'])) 			? $options['maxSize'] 		: self::_maxSize_;
        $extensions 	= (isset($options['extensions']) && is_array($options['extensions'])) 	? $options['extensions'] 	: self::_extensions_;
		
        $mimeTypes 		= (isset($options['mimeTypes']) && is_array($options['mimeTypes'])) 	? $options['mimeTypes'] 	: self::_mimeTypes_;
...
//            echo "\n\n";
            ...
            if (in_array($ext, $extensions) && in_array($type, $mimeTypes) && $size <= $maxSize) 
            {
                if (!file_exists($fileName)) 
                {
                    ...
                    {
                        throw new \Exception( ucfirst(Lang::get("error.msg_11")) );
                        //return false;
                    }
                } 
                else 
            ...
            {
                throw new \Exception( ucfirst(Lang::get("error.msg_12")) );
                //return false;
            }
        } 
        else 
        { 
            //File index does not exist or file uploaded with error
            throw new \Exception( ucfirst(Lang::get("error.msg_14")) );
            //return false;
        }
    }
    ...
    public static function uploadMultipleFiles($index, $baseDir = '', $filePrefix = '', $fileNames = array(), $options = array()) {
        ...
        $maxSize = (isset($options['maxSize']) && is_int($options['maxSize'])) ? $options['maxSize'] : 31457280;
